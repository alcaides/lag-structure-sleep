...
for j=1:nlabelSleepData
    ...
    if strcmp(Ds{1}.(labelGroupName).props.title{:},'Base synset labels')
        for k=1:length(Ds{1}.(labelGroupName).props.dataNames)
            cnt=cnt+1;
            iDs.labels_type{cnt}=['Synset_',Ds{1}.(labelGroupName).props.dataNames{k}];
...
for j=1:nlabelPerceptionData
    ...
    if strcmp(Dp{1}.(labelGroupName).props.title{:},'Base synset labels')
        for k=1:length(Dp{1}.(labelGroupName).props.dataNames)
            cnt=cnt+1;
            iDp.labels_type{cnt}=['Synset_',Dp{1}.(labelGroupName).props.dataNames{k}];
