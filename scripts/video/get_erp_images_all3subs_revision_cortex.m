% function get_erp_images_all3subs_revision_cortex()
clc
...
%% imagesc
cd(fullfile(figures_folder,'video'))
...
opts.Color = 'RGB';
opts.FontSize = 8;
exportfig(gcf,'ROIvsTime_all3.eps',opts);
...
opts.Color = 'RGB';
opts.FontSize = 8;
exportfig(gcf,['ROIvsTimezoomall3_',['subject' num2str(iSub)],'.eps'],opts);
...
exportfig(gcf,'timecourse_all3.eps',opts);
...
set(gca,'yTickLabels',[])
axis off
exportfig(gcf,['timecourseTHALall3',['subject' num2str(iSub)],'.eps'],opts);


cd(fullfile(MAIN_DIRECTORY,'results','video'))
save('timeSeries','Naverage','Eaverage')
%% individual nets
...
for iNet = 1:size(N,1)
    ...
    set(gcf,'Position',[    678   578   686   400]);
    exportfig(gcf,['net_' num2str(iNet),'_all3.eps'],opts);
